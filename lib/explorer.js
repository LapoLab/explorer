...
module.exports = {
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      console.log(vout[i].scriptPubKey.type);
      if (vout[i].scriptPubKey.type != 'nonstandard' &&
        ...
        if (vout[i].scriptPubKey.type == 'zerocoinmint') {
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: 'zLAX', amount: amount_sat});
              loop.next();
            });
        } else {
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, function(tx){
        if (tx) {
          if (!tx.vout) {
            console.log("WARNING: no vout for txid " + input.txid);
            addresses.push({hash: 'zLAX', amount: 0});
            return cb(addresses);
          } else {
...
//		console.log(i);
              if (tx.vout[i].n == input.vout) {
                ...
                if (tx.vout[i].scriptPubKey.addresses) {
                  ...
                  console.log("WARNING no addresses");
                  //addresses.push({hash: "zlax", amount: tx.vout[i].value});
                }
                loop.break(true);
                loop.next();
                //});
